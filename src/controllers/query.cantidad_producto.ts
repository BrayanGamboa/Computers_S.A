        ...
        try {
            ...
            if (result.rowCount <= 5) {
                ...
                if (result.rows[0].cantidad < cantidad) {
                    return h.response({message: `No se tienen la cantidad de articulos solicitados para el producto ${result.rows[0].nombre}`}).code(406);
                } else {
                    ...
                    return h.response({message: "Cantidad de producto guardada con exito"}).code(200);
                }
            } else {
                return h.response({message: `No se pueden agregar más productos al pedido con id: ${id_pedido}`}).code(401);
            }
        } catch (error: any) {
            return h.response({error: error}).code(400);
        } finally {
